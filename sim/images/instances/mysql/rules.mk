...
qemu-mysql-server: $(mysql_server_disk_image)
	...
	-netdev bridge,id=net-bridge,br=$(call confget,host,.host.bridges.instances.name) \
	-device virtio-net-pci,netdev=net-bridge,mac=$(call confget,host,.host.qemu_mac_list[0]) \
	-boot c \
...
qemu-mysql-client: $(mysql_client_disk_image)
	...
	-drive file=$<,media=disk,format=qcow2,if=ide,index=0 \
	-netdev bridge,id=net-bridge,br=$(call confget,host,.host.bridges.instances.name) \
	-device virtio-net-pci,netdev=net-bridge,mac=$(call confget,host,.host.qemu_mac_list[1]) \
	-boot c \
	-display none -serial mon:stdio
