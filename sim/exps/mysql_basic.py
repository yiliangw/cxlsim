    ...
    def prepare_pre_cp(self) -> tp.List[str]:
        ...
        if CONFIG.ssh_control:
            ...
            cmds += [
                ...
                'do sleep 5; done',
                'while ! openstack compute service lsit | grep compute2 | grep -q enabled.*up; '
                'do sleep 5; done',
                # Start the instances
                'openstack server start server client',
                'sleep 60', 
                f'while ! ssh {self.server_ip} true; do sleep 5; done',
                f'while ! ssh {self.client_ip} true; do sleep 5; done',
                # Do a warmup run
...
compute2 = HostSimCls(compute2_config)
